"""
lnterqo v3 — Rolling Walk-Forward Validation (Fixed Risk Profile)
=================================================================
Same rolling WFV methodology as v2 but using v1's fixed risk tiers.
Direct A/B comparison: EG sizing (v2) vs fixed sizing (v3).

  - 5 anchored rolling windows (expanding IS, fixed 10% OOS per window)
  - Per-window: grid search → best variation → frozen ML → OOS
  - Fixed risk tiers: 0.5% default, 1% for confidence 4-5 (v1 profile)
  - Loss ladder: 0.5% → 0.25% after 3 losses, block after 5 losses
...
def _run_is_grid(df_daily_is, df_5m_is, window_idx):
    ...
    for label, min_rr, zone_lb, cisd_lb in VARIATIONS:
        # Raw IS run (no ML)
        trades_raw, eq_raw = run_lnterqo_backtest_v3(
        ...
        trades_ml, eq_ml = run_lnterqo_backtest_v3(
            "Gold", df_daily_is, df_5m_is,
            min_rr=min_rr, zone_lookback=zone_lb, cisd_lookback=cisd_lb,
            force_neutral_bias=True,
...
def main():
    print(f"\n{'='*70}")
    print("  lnterqo v3 — ROLLING WFV  (Fixed Risk Profile — v1 tiers)")
    print(f"  Data: {COMBINED_5M.name}")
    print(f"  Windows: {N_WINDOWS}  |  IS: anchored  |  OOS per window: {OOS_WINDOW_PCT*100:.0f}%")
    print(f"  Sizing: Fixed tiers — 0.5% default, 1% conf≥4, loss ladder")
    print(f"{'='*70}")
    ...
    for w, (is_end_bar, oos_end_bar) in enumerate(windows):
        ...
        trades_oos, equity_oos = run_lnterqo_backtest_v3(
    ...
    df_all.to_csv("backtest/lnterqo_v3_oos_trades.csv", index=False)
    print(f"  Trade log saved: backtest/lnterqo_v3_oos_trades.csv")
    ...
    print(_row("lnterqo v2 (rolling+EG)",
               _LNT_V2["trades"], _LNT_V2["wr"], _LNT_V2["ev_r"],
               _LNT_V2["max_dd"], _LNT_V2["sharpe"], _LNT_V2["barrier"]))
    print(_row("lnterqo v3 (rolling+fixed)",
    ...
    print(f"  - Fixed sizing: 0.5% default, 1% conf≥4, loss ladder (v1 profile)")
